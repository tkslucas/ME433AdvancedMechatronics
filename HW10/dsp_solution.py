...
def plot_results(raw_time, raw_signal, filtered_time, filtered_signal, filter_name, params):
    # Calculate FFTs
...
def MAF(address, window_size):
    ...
    plot_results(time, signal, filtered_time, filtered_signal, "MAF", f"window={window_size}")
...
def IIR(address, weight_A, weight_B):
    ...
    plot_results(time, signal, time, filtered_signal, "IIR", f"A={weight_A}, B={weight_B}")
...
def FIR(address, coefficients, method):
    ...
    filtered_time = np.linspace(time[0], time[-1], len(filtered_signal))
    plot_results(time, signal, filtered_time, filtered_signal, "FIR", f"{method} ({len(coefficients)} taps)")
